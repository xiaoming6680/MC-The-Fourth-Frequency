...
本文描述 `0.3.0-beta` 当前实现，不再把 Alpha 阶段的四阶段“异象主体”终局当成现行架构。
...
| 情境 | 结果 |
| --- | --- |
| 主菜单／无玩家 | `music_menu`（4 首） |
| 常规游戏 | `music_game`（3 首，沿用原版 10–20 分钟间隔与音乐频率选项） |
| 世界接口成功结算、传送门开启、终末之诗／制作人员名单 | `music_ending`（1 首，循环） |
| 世界接口召唤与三个阶段、失败结算 | 静音 |
| 其他带音乐标记的 Boss 血条（末影龙、凋灵） | 静音 |
| 追逐战 | 预警期开始淡出，黑屏后硬切 |

静音走的是增益淡出而非直接停止，所以进入 Boss 战或追逐预警时音乐是退场而不是被掐断；
下一首同样从零增益淡入。追逐黑屏是唯一的硬切点——那里的设定是信号被切断。

音频素材以 44.1 kHz 立体声 Ogg Vorbis（q4）随模组分发，母带增益在 `tools/import_music.py`
导入时按 70% 烘焙进文件，而不是写在 `sounds.json` 里。
